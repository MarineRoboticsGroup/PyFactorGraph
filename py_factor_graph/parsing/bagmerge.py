...
def parse_args():
    parser = argparse.ArgumentParser(
        prog="bagmerge.py", description="Merges two bagfiles."
    )
    parser.add_argument(
        "-o",
        type=str,
    ...
    parser.add_argument(
        "-i", help="reindex bagfile", default=False, action="store_true"
    )
    parser.add_argument(
        "main_bagfile",
        type=str,
...
def get_next(
    bag_iter, reindex=False, main_start_time=None, start_time=None, topics=None
):
    try:
        ...
        if reindex:
            return (result[0], result[1], result[2] - start_time + main_start_time)
        return result
...
def merge_bag(main_bagfile, bagfile, outfile, topics=None, reindex=True):
    """Merges two bagfiles into a new bagfile.

    Args:
        main_bagfile (str): the first bagfile
        bagfile (str): the second bagfile
        outfile (str): the name of the output bagfile
        topics (List[str], optional): If specified, the list of topics to write
        in the merge. Defaults to None.
        reindex (bool, optional): Whether to reindex the bags while merging. Defaults to True.
    """
...
if __name__ == "__main__":
    ...
    if args.t != None:
        args.t = args.t.split(",")
    merge_bag(
        args.main_bagfile, args.bagfile, outfile=args.o, topics=args.t, reindex=args.i
    )
